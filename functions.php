<?php
     ...
     */
	define('STAGING_TOKEN', 'accd2be9394b28d536e47d48bef0f0c1');
	define('PROD_TOKEN', '9837a8f33d16723c9f620b42bbe05023');
    if(!isset($_SESSION)){
        session_start();
    }
    define('DEV_ENV', 0);
    ...
    function clio_send( $entry, $form ) {
        ...
        if(!empty($entry['33'])){
            $message .= "<br/> Hearing Date: ".date('m-d-Y', strtotime($entry['33']))."<br/> ------------------------------------------------------------------------";
        }
        if(!empty($entry['36'])){
